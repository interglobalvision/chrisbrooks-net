...
function cmb2_attached_posts_fields_render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
  ...
  $has_thumbnail = $field->options( 'show_thumbnails' ) == TRUE ? ' has-thumbnails' : '';
    ...
    // Set thumbnail if the options is true
	  $thumbnail = $field->options( 'show_thumbnails' ) == TRUE ? get_the_post_thumbnail( $post->ID, 'admin-gallery-thumb' ) : '';
...
function cmb2_attached_posts_fields_display_attached( $field, $attached ) {
    ...
    // Set thumbnail if the options is true
	  $thumbnail = $field->options( 'show_thumbnails' ) == TRUE ? get_the_post_thumbnail( $post_id, 'admin-gallery-thumb' ) : '';
